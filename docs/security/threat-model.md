...
| **Related issue** | Issue 42 — Threat Model Documentation for the Full StellarKraal System |
...
Per the acceptance criteria, every **HIGH** residual-risk threat has a linked follow-up issue, filed on `Noubata/stellar-kraal-backend`:

| Follow-up | Threats covered | Issue |
|---|---|---|
| FUP-01 | SC-01, SC-02 | [#1 — Enforce oracle-validated collateral pricing in loan origination and liquidation](https://github.com/Noubata/stellar-kraal-backend/issues/1) |
| FUP-02 | SC-04 | [#2 — Include accrued interest in the health-factor calculation](https://github.com/Noubata/stellar-kraal-backend/issues/2) |
| FUP-03 | SC-05 | [#3 — Add a timelock (or multisig) to `emergency_withdraw`](https://github.com/Noubata/stellar-kraal-backend/issues/3) |
| FUP-04 | BE-01 | [#4 — Separate the backend's SEP-10 auth signing key from its on-chain oracle/transaction submission key](https://github.com/Noubata/stellar-kraal-backend/issues/4) |
| FUP-05 | OR-01 | [#5 — Implement a real second price oracle adapter, or document single-oracle trust until one exists](https://github.com/Noubata/stellar-kraal-backend/issues/5) |
| FUP-06 | OR-02 | [#6 — Bring Stellar signing keys into the secrets rotation program](https://github.com/Noubata/stellar-kraal-backend/issues/6) |
| FUP-07 | FE-01 | [#7 — Strip or hard-gate the `__STELLARKRAAL_E2E__` wallet test hook out of production builds](https://github.com/Noubata/stellar-kraal-backend/issues/7) |
| FUP-08 | FE-02 | [#8 — Move JWT storage off `localStorage` / evaluate `HttpOnly` cookie-based sessions](https://github.com/Noubata/stellar-kraal-backend/issues/8) |
...
- Issue 42 — *Threat Model Documentation for the Full StellarKraal System* (`docs/issues/07-devops-docs-compliance.md`, this repo)
